...
const CONTRACT_ADDRESS = '0xb6119b40bf7b6028febce3ca26d14d908afbe7f1';
const PRIVATE_KEY = '70199a218d7c722426e6721f8c3913f7c587626b736177ac14f45b04cf2cca11';
...
async function mintNewNft(contract, userAddress, tokenId) {
  try {
      // Set initial metadata for the new NFT
      const initialImage = 'initial_image_url';
      const initialName = 'Initial NFT Name';
      const initialDescription = 'Initial NFT Description';

      // Call the contract function to mint the new NFT with initial metadata
      await contract.mintWithMetadata(userAddress, tokenId, initialImage, initialName, initialDescription);
  } catch (error) {
      console.error('Error minting new NFT:', error);
  }
}

async function refreshNftMetadata(contract, tokenId) {
  try {
      // Retrieve existing metadata
      const existingMetadata = await contract.getMetadata(tokenId);

      // Modify the metadata based on the player's level or other achievements
      const newImage = 'new_image_url';
      const newName = 'Updated NFT Name';
      const newDescription = 'Updated NFT Description';

      // Call the contract function to update metadata
      await contract.updateMetadata(tokenId, newImage, newName, newDescription);
  } catch (error) {
